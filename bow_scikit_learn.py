...
def process_pipeline(train_x, train_y):
...
def grid_search(text_clf, train_x, train_y):
    ...
    gs_clf = gs_clf.fit(train_x[:400], train_y[:400])
...
def main(argc, argv):
    if argc < 3:
        print('Usage:%s <train_dir> <test_dir> <cv>' % (argv[0]))
        exit(1)
    ...
    process_pipeline(train_x, train_y)
