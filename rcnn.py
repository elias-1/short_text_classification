...
tf.app.flags.DEFINE_string('test_data_path', "data/test/normal_test.txt",
                           'Test data dir')
tf.app.flags.DEFINE_string('log_dir', "rcnn_logs", 'The log  dir')
...
class Model:
    def __init__(self, c2vPath, numHidden):
        ...
        with tf.variable_scope('Transform') as scope:
            self.transform_W = tf.get_variable(
                "transform_W",
                shape=[
                    1, 1, FLAGS.embedding_size + self.numHidden * 2,
                    self.numHidden * 2
                ],
    ...
    def inference(self, clfier_wX, reuse=None, trainMode=True):
        ...
        left_self_right = tf.concat(
            [forward_output, word_vectors, backward_output], 2)

        hidden = tf.reshape(left_self_right, [
            -1, FLAGS.max_sentence_len, 1,
            FLAGS.embedding_size + self.numHidden * 2
        ])
